    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Add Machine (next step)')),
        ),
      ...
      body: ListView(
        ...
        children: [
          ...
          const SectionHeader(
            title: 'List',
            subtitle: 'Block machines snapshot (UI-only)',
          ),
          ...filtered.map(
            (m) => Padding(
              ...
              child: Card(
                child: ListTile(
                  ...
                  trailing: StatusChip(
                    status: m.$4,
                    labelOverride: m.$4 == UiStatus.ok ? 'Running' : 'Idle',
                  ),
                  onTap: () => ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('Open machine: ${m.$1} (next step)'),
                    ),
                  ),
                ),
              ),
